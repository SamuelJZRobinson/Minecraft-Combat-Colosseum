...
# Gamemodes Optional Inits
  # Spawn Koth Hill
  execute if score gamemode Settings matches 3 run function gc:states/8_round/gamemodes/koth/init
  execute if score gamemode Settings matches 4 run function gc:states/8_round/gamemodes/koth/init
...
# Settings
  ...
  # Damage Types
    ...
    execute if score doFriendlyFire Settings matches 0 run function gc:states/6_config_gameplay/set/set_teams_teams_friendly_fire_false
    execute if score doFriendlyFire Settings matches 1 run function gc:states/6_config_gameplay/set/set_teams_teams_friendly_fire_true

# Players
  # Clear Deaths
  scoreboard players reset * PlayerDeaths
  # Teleport
  execute if score teamCount Settings matches 1 if score isSolo Settings matches 1 run function gc:states/6_config_gameplay/tp/tp_team_solo
  execute if score teamCount Settings matches 1 if score isSolo Settings matches 0 run function gc:states/6_config_gameplay/tp/tp_team_ffa
  execute if score teamCount Settings matches 2.. run function gc:states/6_config_gameplay/tp/tp_team_teams
  # Heal
  effect give @a[team=!Lobby] instant_health 1 20
  # Give Gear
  execute as @a[team=!Lobby,gamemode=!spectator] run function gc:states/8_round/respawn/give_gear
...
# Set Scoreboards
function gc:states/6_config_gameplay/set/manage_scoreboards
