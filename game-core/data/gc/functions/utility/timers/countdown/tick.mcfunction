...
# Final Countdown
execute if score minutes TimerCountdown matches 0 if score seconds TimerCountdown <= COUNTDOWN_SECONDS_START TimerCountdown if score seconds TimerCountdown matches 0.. run function gc:utility/timers/countdown/final_countdown

# Loop Tick
execute if score seconds TimerCountdown matches 1.. run schedule function gc:utility/timers/countdown/tick 1s replace

# Proceed
execute if score isGameStarted GameStatus matches 1 if score minutes TimerCountdown matches 0 if score seconds TimerCountdown matches ..0 run schedule function gc:states/inc_state 1t replace