# Game Settings
fill 246 -29 -153 248 -29 -153 minecraft:jungle_wall_sign[facing=south] destroy
fill 246 -30 -153 246 -30 -153 minecraft:jungle_wall_sign[facing=south] destroy

# Player Settings
fill 252 -29 -153 254 -29 -153 minecraft:jungle_wall_sign[facing=south] destroy

# Environment Settings
fill 258 -29 -153 260 -29 -153 minecraft:jungle_wall_sign[facing=south] destroy

# Damage Type Settings
fill 264 -29 -153 266 -29 -153 minecraft:jungle_wall_sign[facing=south] destroy
fill 264 -30 -153 264 -30 -153 minecraft:jungle_wall_sign[facing=south] destroy

# Root
fill 257 -28 -142 256 -28 -142 minecraft:jungle_wall_sign[facing=north] destroy
setblock 256 -29 -142 minecraft:jungle_wall_sign[facing=north] destroy