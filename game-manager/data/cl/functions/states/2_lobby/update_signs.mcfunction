# Notes
  # Gamemode must have first priority to set values correctly.

# Root
# function cl:states/2_lobby/merge/merge_highscores
function cl:states/2_lobby/merge/merge_credits
function cl:states/2_lobby/merge/merge_ideal_settings
function cl:states/2_lobby/merge/merge_start

# Game Settings
function cl:states/2_lobby/merge/merge_gamemode
function cl:states/2_lobby/merge/merge_class
function cl:states/2_lobby/merge/merge_team_count
function cl:states/2_lobby/merge/merge_grace_period

# Player Settings
function cl:states/2_lobby/merge/merge_respawn_time
function cl:states/2_lobby/merge/merge_natural_regeneration
function cl:states/2_lobby/merge/merge_unbreakable_tools

# Environment Settings
function cl:states/2_lobby/merge/merge_scenery
function cl:states/2_lobby/merge/merge_edge_trap
function cl:states/2_lobby/merge/merge_time_of_day

# Damage Types
function cl:states/2_lobby/merge/merge_drowning_damage
function cl:states/2_lobby/merge/merge_fire_damage
function cl:states/2_lobby/merge/merge_fall_damage
function cl:states/2_lobby/merge/merge_friendly_fire